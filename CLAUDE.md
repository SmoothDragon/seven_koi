...
6. **Endgame:** when replenishing hits an **empty** deck, put **every** remaining facedown card onto the tableau in one sweep (**`|spread| < L₀`** is OK — **no facedown stubs** besides cards players already claimed). XOR conservation still matches Lemma E (**`math/NOTES.md` §7.1**); unnoticed legal matches can still lurk (**`math/RESULTS.md` §3**). Keep claiming while anyone spots a legal 4-card match. Once **everyone agrees** **no matches remain**, **end**.
