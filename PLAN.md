...
2. **Dealing-math reconciliation — recommended playable procedure** (`math/NOTES.md` §6 vs **[CP24]** in `math/NOTES.md` / `CLAUDE.md`):
   ...
   - **Endgame — maximal tableau then consensus stop.** When the facedown deck is **empty** after a replenish pass, deal **every** remaining stock card onto the tableau if any were still boxed out by **partial** draws — the tableau should hold **all** cards not sitting in players' claimed piles (**as full a spread as the remaining deck allows**). Play continues (**real-time** claims) whenever someone spots a legal 4-card match. When **everyone agrees** no further matches are visible, **the game ends** — cards left face-up score for **nobody**.
