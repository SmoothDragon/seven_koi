...
Together, **`PLAN.md`** escalation caps **`13`** (standard, **`GF(2)^7`**) and **`10`** (beginner, **`GF(2)^6`**) align with **`[CP24]`**: **`|L| ≥ 13`** / **`|L| ≥ 10`** force **some** cancelling quadruple among **distinct** layouts inside the **full** vector space. Baseline **`L₀ = 10`** (seven koi) rests on the **restricted** **`D ⊆ GF(2)^7`**: Lemma D gives **`s_max(D) ≥ 9`**, greedy search corroborates **`≤ 9`**, hence **`10`** tableau cards suffice **provided** **`s_max(D)=9`** holds. Baseline **`L₀ = 8`** (six koi **`|D| = 32`**) rests on analogous **deck-local empirical** Mode B stalls (**`math/layout_stall_sweep.py`**, **`math/RESULTS.md`**): **`L = 7` stalls badly**, **`L ≥ 8`** did not stall in sampled runs; escalating to **`10`** recovers the **ambient** existential guarantee referenced above without leaning on unpublished **`s_max`** for **`D`** in dimension six.
